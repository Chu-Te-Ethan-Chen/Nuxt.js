    ...
    actions: {
      ...
      authenticateUser(vuexContext, authData) {
        ...
        return this.$axios
          ...
          .then((res) => {
            console.log(res)
            const expiredDate = new Date().getTime() + Number.parseInt(res.expiresIn) * 1000
            vuexContext.commit('setToken', res.idToken)
            localStorage.setItem('token', res.idToken)
            localStorage.setItem('tokenExpiration', expiredDate)
            Cookie.set('jwt', res.idToken)
            Cookie.set('expirationDate', expiredDate)
          })
          .catch((e) => {
      ...
      initAuth(vuexContext, req) {
        ...
        if (req) {
          ...
          expirationDate = req.headers.cookie.split(';').find((c) => {
            c.trim().startsWith('expirationDate=').split('=')[1]
          })
